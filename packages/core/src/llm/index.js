 ...
 * 2. Auto-detect available providers (Ollama → OpenAI → Mock)
 *
 ...
 *
 * Detection priority (favoring local):
 * 1. Ollama (local, free, open source)
 * 2. OpenAI (commercial fallback)
...
export class AutoDetectLLMProvider extends BaseLLMProvider {
  ...
  async _detect() {
    ...
    this._detectPromise = (async () => {
      ...
      // 2. Check OpenAI (commercial fallback)
      ...
      console.error('[LLMProvider]   Install Ollama: https://ollama.ai');
