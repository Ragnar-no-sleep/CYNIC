...
- [x] Discord notifications - DiscordService with webhook + bot modes
- [ ] Linear/Jira task sync - Future (lower priority)
...
- `packages/mcp/src/slack-service.js` - Slack API integration
- `packages/mcp/src/discord-service.js` - Discord integration with:
  - Webhook mode (simple, no bot needed) - just set DISCORD_WEBHOOK_URL
  - Bot mode (advanced) - DISCORD_BOT_TOKEN + DISCORD_CHANNEL_ID
  - Rich embeds with colors, fields, timestamps
  - Same API as Slack: sendJudgment, sendDangerAlert, sendPatternDetected, sendSessionSummary
  - Rate limiting (30/min) with queuing
- `packages/mcp/src/tools/domains/notifications.js` - MCP tools:
  - `brain_notifications` - Internal notification management
  - `brain_slack_send` / `brain_slack_status` - Slack integration
  - `brain_discord_send` / `brain_discord_status` - Discord integration

### I4.2 LLM Providers ✅ 90%
- [x] Ollama embeddings - OllamaEmbedder (nomic-embed-text, mxbai-embed-large, all-minilm)
- [x] OpenAI embeddings - OpenAIEmbedder (text-embedding-ada-002)
- [x] Auto-detection embeddings - AutoDetectEmbedder tries Ollama first, falls back to Mock
- [x] LLM Provider abstraction - Unified interface for judgment LLMs
- [x] Ollama judgment - OllamaLLMProvider (llama3, mistral, qwen, deepseek-r1)
- [x] OpenAI judgment - OpenAILLMProvider (gpt-4o, gpt-4o-mini)
- [x] Auto-detection judgment - Ollama → OpenAI → Mock fallback
- [ ] Anthropic API direct (for agent hooks) - Deferred (Claude via Claude Code)

**Implemented:**
- `packages/persistence/src/services/embedder.js` - Embedding providers
- `packages/core/src/llm/index.js` - LLM Provider abstraction:
  - `OllamaLLMProvider` - Local inference (llama3, mistral, qwen, deepseek-r1)
  - `OpenAILLMProvider` - Cloud inference (gpt-4o, gpt-4o-mini)
  - `MockLLMProvider` - Testing
  - `AutoDetectLLMProvider` - Tries Ollama → OpenAI → Mock
  - `createLLMProvider()` / `getLLMProvider()` - Factory + singleton
  - Cost tracking per model
  - φ⁻¹ confidence cap on all providers
...
| External Integrations | 20% | 70% | ✅ 85% |
